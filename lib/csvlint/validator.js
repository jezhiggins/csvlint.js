...
class Validator {
  ...
  async validateStream (sourceStream) {
    for await (const line of chunksToLines(sourceStream)) {
      ...
      try {
        ...
        this.buildExceptionMessage(e, this.currentLine_, line)
      }
    } // for ...
  } // parseLine

  validateLine (record, currentLine = null) {
    const row = record[0].filter(col => col)
  ...
  buildExceptionMessage (exception, currentLine, badLine) {
    const error = CsvParseErrors.translate(exception)
    this.errors_.buildError(error, "structure", currentLine, null, badLine)
