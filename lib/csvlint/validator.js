...
class Validator {
  ...
  async validateStream (sourceStream) {
    const parser = csvparse({
      ...
      on_record: (record, { lines }) => {
        this.validateLine(record, lines, currentLine)
  ...
  validateLine (record, lineNumber = null, lineContents) {
    ...
    if(this.schema_) {
      this.schema_.validateRow(row, lineNumber, [], this.source_, this.validate_) // not entirely sure what allErrors is doing here
...
const presentOrAbsent = /header=(present|absent)/
