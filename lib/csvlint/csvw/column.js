...
class Column {
  ...
  static fromJson (number, column_desc, base_url=nil, lang="und", inherited_properties={}) {
    const annotations = {}
    const warnings = []
    ...
    for (const [property, value] of Object.entries(column_desc)) {
      if (property === "@type") {
        ...
        warning.forEach(w => this.warning(w, 'metadata', `${property}: ${value}`))
        ...
        else this.warning('invalid_property', 'metadata', `column: ${property}`, null)
      }
    } // for ...
