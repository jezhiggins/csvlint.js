...
function toDateTime (value, constraints) {
  const datePattern = constraints.datePattern || 'yyyy-MM-dd!HH:mm:ssZ'
  const dateTime = DateTime.fromISO(value)
  const str = dateTime.toFormat(datePattern)
    .replace('!', 'T')
    .replace(/-0$/, "Z")
...
function toDate (value, constraints) {
  const datePattern = constraints.datePattern || 'yyyy-MM-dd'
  const date = DateTime.fromString(value, datePattern)
  const str = date.toFormat(datePattern)
  if (str !== value) throw new TypeError()
  return value
}
function toTime (value, constraints) {
  const timePattern = constraints.datePattern || 'HH:mm:ss'
  const time = DateTime.fromString(value, timePattern)
  const str = time.toFormat(timePattern)
  if (str !== value) throw new TypeError()
  return value
}
