...
function uriTemplateProperty (type) {
  ...
  //  return URITemplate.new(value), null, type if isString(value)
...
function datatypeProperty (value, base_url, lang) {
  ...
  if (!STRING_DATATYPES.includes(datatype.base) && !BINARY_DATATYPES.includes(datatype.base)) {
...
function nameProperty (value, base_url, lang) {
  return (isString(value) && NAME_REGEXP.test(value))
    ? [value, null, 'column']
    : [null, 'invalid_value', 'column']
} // nameProperty

function transformationsProperty (value, base_url, lang) {
  const transformations = []
  const warnings = []
  if (Array.isArray(value)) {
    for (const transformation of value) {
      if (isObject(transformation)) {
        for (const [p, v] of Object.entries(transformation)) {
          if (p === '@id') {
            metadataErrorIf(/^_:/.test(v), 'transformations[#{i}].@id', '@id starts with _:')
          } else if (p === '@type') {
            metadataErrorIf(v !== 'Template', 'transformations[#{i}].@type', "@type of transformation is not 'Template'")
          } else if (p === 'url') {
          } else if (p === 'titles') {
          } else {
            const [v, warning, type] = checkProperty(p, v, base_url, lang)
            // value.delete(p)  IS THIS CORRECT?
            if (type === 'transformation') {
              warnings.push('invalid_property')
            }
            warnings.push(...asArray(warning))
          }
        }
        transformations.push(transformation)
      } else {
        warnings.push('invalid_transformation')
      }
    }
  } else {
    warnings.push('invalid_value')
...
function tableDirectionProperty (value, base_url, lang) {
  return ['ltr', 'rtl', 'auto'].includes(value)
    ? [value, null, 'table']
    : ['auto', 'invalid_value', 'table']
} // tableDirectionProperty

function tableSchemaProperty (value, base_url, lang) {
  let schema_base_url = base_url
  let schema_lang = lang
  let schema = {}
  if (isString(value)) {
    schema_url = (new URL(value, base_url)).href
    schema_base_url = schema_url
    const schema_ref = schema_url // schema_url.start_with?("file:") ? File.new(schema_url[5..-1]) : schema_url  TODO! resolver
    schema = JSON.parse(open(schema_ref).read)
    schema['@id'] = schema['@id'] ? new URL(schema['@id'], schema_url).href : schema_url
    if (schema['@context']) {
      if (Array.isArray(schema['@context']) && (schema['@context'].length > 1)) {
    ...
    schema = Object.assign({}, value.clone)
  } else {
    return [{}, 'invalid_value', 'table']
  }
  const warnings = []
  for (const [p, v] of schema) {
    if (p === '@id') {
      metadataErrorIf(/^_:/.test(v), 'tableSchema.@id', '@id starts with _:')
    } else if (p === '@type') {
      metadataErrorIf(v != 'Schema', 'tableSchema.@type', "@type of schema is not 'Schema'")
    } else {
      const [v, warning, type] = checkProperty(p, v, schema_base_url, schema_lang)
      if ((type === 'schema' || type === 'inherited') && !warning.length) {
        schema[p] = v
      } else {
        delete schema[p]
        warnings.push('invalid_property')
...
function encodingProperty (value, base_url, lang) {
  return VALID_ENCODINGS.includes(value)
    ? [value, null, 'dialect']
    : [null, 'invalid_value', 'dialect']
} // encodingProperty

function trimProperty (value, base_url, lang) {
  if (value === true || value === 'true') value = 'true'
  if (value === false || value === 'false') value = 'false'
  if (value == 'start') value = 'start'
  if (value == 'end') value = 'end'

  return ['true', 'false', 'start', 'end'].includes(value)
    ? [value, null, 'dialect']
    : [true, 'invalid_value', 'dialect']
} // trimProperty

function foreignKeysProperty (value, base_url, lang) {
  const foreign_keys = []
  const warnings = []
  if (Array.isArray(value)) {
    for (let foreign_key of value) {
      if (isObject(foreign_key)) {
        foreign_key = Object.assign({}, foreign_key)
        for (const [p, v] of foreign_key) {
          const [v, warning, type] = checkProperty(p, v, base_url, lang)
          if (type === 'foreign_key' && !warning.length) {
            foreign_key[p] = v
          } else if (/:/.test(p)) {
            metadataError(`foreignKey.${p}`, 'foreignKey includes a prefixed (common) property')
          } else {
            delete foreign_key[p]
            if (type !== 'foreign_key') warnings.push('invalid_property')
            warnings.push(...asArray(warning))
          }
        }
        foreign_keys.push(foreign_key)
      } else {
        warnings.push('invalid_foreign_key')
      }
    }
  } else {
    warnings.push('invalid_value')
...
function referenceProperty (value, base_url, lang) {
  if (isObject(value)) {
    const warnings = []
    for (const [p, v] of Object.entries(value)) {
      if (['resource', 'schemaReference', 'columnReference'].includes(p)) {
        const [v, warning, type] = checkProperty(p, v, base_url, lang)
        if (!warning.length) {
          value[p] = v
        } else {
    ...
    metadataErrorIf(!value.columnReference, 'foreignKey.reference.columnReference', 'foreignKey reference columnReference is missing')
    metadataErrorIf(!(value.resource || value.schemaReference), 'foreignKey.reference', 'foreignKey reference does not have either resource or schemaReference')
    metadataErrorIf(value.resource && value.schemaReference, 'foreignKey.reference', 'foreignKey reference has both resource and schemaReference')
...
const PROPERTIES = {
  ...
  transformations: transformationsProperty,
  tableDirection: tableDirectionProperty,
  ...
  trim: trimProperty,
  // schema properties
  columns: (value, base_url, lang) => [value, null, 'schema'],
  primaryKey: columnReferenceProperty('schema'),
  foreignKeys: foreignKeysProperty,
  rowTitles: columnReferenceProperty('schema'),
  // transformation properties
  targetFormat: (value, base_url, lang) => [value, null, 'transformation'],
  scriptFormat: (value, base_url, lang) => [value, null, 'transformation'],
  source: (value, base_url, lang) => [value, null, 'transformation'],
  // foreignKey properties
  columnReference: columnReferenceProperty('foreign_key'),
  reference: referenceProperty,
  // foreignKey reference properties
  resource: (value, base_url, lang) => [value, null, 'foreign_key_reference'],
  schemaReference: (value, base_url, lang) => [new URL(value, base_url).href, null, 'foreign_key_reference']
}
...
const BCP47_REGEXP = BCP47_LANGUAGETAG_REGEXP
...
function checkPropertyWrapper (property, value, base_url, lang) {
  const [v, warnings, type] = checkProperty(property, value, base_url, lang)
  return [v, asArray(warnings), type]
} // checkPropertyWrapper

module.exports = checkPropertyWrapper
