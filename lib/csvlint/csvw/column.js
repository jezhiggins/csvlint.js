...
class Column {
  ...
  validateLength (value, row) {
    const { dLength, dMinLength, dMaxLength } = this.datatype_

    if (!dLength && !dMinLength && !dMaxLength)
      return true

    let length = value.length
    if (this.isBase64())
      length = value.replace(/==?$/, '').length * 3 /4
    if (this.isHexBinary())
      length = value.length / 2

    let valid = true
    if (dMinLength && length < dMinLength) {
      this.error('min_length', row, this.number_, value, { minLength: dMinLength })
      valid = false
    }
    if (dMaxLength && length > dMaxLength) {
      this.error('max_length', row, this.number_, value, { maxLength: dMaxLength })
      valid = false
    }
    if (dLength && length != dLength) {
      this.error('length', row, this.number_, value, { length: length })
      valid = false
    }
    return valid
  } // validateLength
