...
const PROPERTIES = {
  ...
  // inherited properties
          ...
          "separator": lambda { |value, base_url, lang|
            return value, null, :inherited if value.instance_of?(String) || value.null?
            return null, 'invalid_value', :inherited
          },
          ...
          "datatype": lambda { |value, base_url, lang|
            ...
            if value.instance_of? Hash
              if value["@id"]
                metadataError("datatype.@id", `"datatype @id must not be the id of a built-in datatype (${value["@id"]})"` if BUILT_IN_DATATYPES.values.include?(value["@id"])
            ...
            unless STRING_DATATYPES.include?(value["base"]) || BINARY_DATATYPES.include?(value["base"])
              metadataError("datatype.length", `datatypes based on ${value["base"]} cannot have a length facet` if value["length"]
              metadataError("datatype.minLength", `datatypes based on ${value["base"]} cannot have a minLength facet` if value["minLength"]
              metadataError("datatype.maxLength", `datatypes based on ${value["base"]} cannot have a maxLength facet` if value["maxLength"]
            end
            ...
            warnings += convertValueFacet(value, "minInclusive", value["base"])
            warnings += convertValueFacet(value, "minExclusive", value["base"])
            warnings += convertValueFacet(value, "maxInclusive", value["base"])
            warnings += convertValueFacet(value, "maxExclusive", value["base"])
            ...
            metadataError("", `datatype cannot specify both minimum/minInclusive (${minInclusive}) and minExclusive (${minExclusive}`) if minInclusive && minExclusive
            metadataError("", "datatype cannot specify both maximum/maxInclusive (${maxInclusive}) and maxExclusive (${maxExclusive}" if maxInclusive && maxExclusive
            metadataError("", "datatype minInclusive (${minInclusive}) cannot be more than maxInclusive (${maxInclusive}" if minInclusive && maxInclusive && minInclusive > maxInclusive
            metadataError("", "datatype minInclusive (${minInclusive}) cannot be more than or equal to maxExclusive (${maxExclusive}" if minInclusive && maxExclusive && minInclusive >= maxExclusive
            metadataError("", "datatype minExclusive (${minExclusive}) cannot be more than or equal to maxExclusive (${maxExclusive}" if minExclusive && maxExclusive && minExclusive > maxExclusive
            metadataError("", "datatype minExclusive (${minExclusive}) cannot be more than maxInclusive (${maxInclusive}" if minExclusive && maxInclusive && minExclusive >= maxInclusive

            metadataError("", "datatype length (${value["length"]}) cannot be less than minLength (${value["minLength"]}" if value["length"] && value["minLength"] && value["length"] < value["minLength"]
            metadataError("", "datatype length (${value["length"]}) cannot be more than maxLength (${value["maxLength"]}" if value["length"] && value["maxLength"] && value["length"] > value["maxLength"]
            metadataError("", "datatype minLength (${value["minLength"]}) cannot be more than maxLength (${value["maxLength"]}" if value["minLength"] && value["maxLength"] && value["minLength"] > value["maxLength"]
            ...
            if value["format"]
              if REGEXP_FORMAT_DATATYPES.include?(value["base"])
                ...
                begin
                  ...
                  value["format"] = Csvlint::Csvw::NumberFormat.new(null, value["format"]["groupChar"], value["format"]["decimalChar"] || ".", INTEGER_FORMAT_DATATYPES.include?(value["base"]))
          ...
          "textDirection": lambda { |value, base_url, lang|
            value = value.to_sym
            return value, null, :inherited if [:ltr, :rtl, :auto, :inherit].include? value
