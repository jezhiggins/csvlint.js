...
class Validator {
  ...
  async validateStream (sourceStream) {
    const parser = csvparse({
      ...
      on_record: (record, {lines}) => {
        this.currentLine_ = lines
        this.validateLine(record)
        this.data.push(record)
      }
    })
    parser.on('skip', err => this.buildExceptionMessage(err))
  ...
  buildExceptionMessage (exception) {
    ...
    this.errors_.buildError(error, "structure", exception.lines, null, null)
