...
describe('Csvlint::Validator', () => {
  before(() => {
    ...
    nock('http://example.com').get('/csv-metadata.json').reply(
        404)
  })
...
/*
  describe('multi line CSV validation with included schema', () => {

  })

  describe('single line row validation with included schema', () => {
...
// TODO ^- -> and its less the size of col_counts than the homogeneity of its contents which is important
    ...
    it('.each() -> parse_contents method validates a well formed CSV', () => {
      // when invoking parse contents
      data = StringIO.new(%Q{"Foo","Bar","Baz"\r\n"1","2","3"\r\n"1","2","3"\r\n"3","2","1"})

      const validator = new CsvlintValidator(data)
...
// TODO in its formats object but is provided with 5 rows (with one nil row) [uses validation_warnings_steps.rb]
      expect(validator.expected_columns).to.eql(3)
      expect(validator.col_counts.count).to.eql(4)
      expect(validator.data.length).to.eql(4)

    })

    it('.each() -> `parse_contents` parses malformed CSV and catches unclosed quote', () => {
      ...
      it('.each() -> `parse_contents` parses malformed CSV and catches stray quote', () => {
        p})ing "cannot make Ruby generate a stray quote error"
      // doesn't build warnings because check_consistency isn't invoked
      ...
      it('.each() -> `parse_contents` parses malformed CSV and catches whitespace and edge case', () => {
      ...
      it('handles line breaks within a cell', () => {
        data = StringIO.new(%Q{"a","b","c"\r\n"d","e","this is\r\nvalid"\r\n"a","b","c"})
        const validator = new CsvlintValidator(data)
        expect(validator.isValid).to.eql(true)
      })

      it('handles multiple line breaks within a cell', () => {
        data = StringIO.new(%Q{"a","b","c"\r\n"d","this is\r\n valid","as is this\r\n too"})
        const validator = new CsvlintValidator(data)
        expect(validator.isValid).to.eql(true)
      })
    ...
    describe('csv dialect', () => {
      ...
      it('File.open.each_line -> `validate` passes a valid csv', () => {
        ...
        const validator = new CsvlintValidator(File.new(file))
    ...
    describe('with a single row', () => {
      ...
      it('validates correctly', () => {
        stream = "\"a\",\"b\",\"c\"\r\n"
        const validator = new CsvlintValidator(StringIO.new(stream), "header" => false)
        expect(validator.isValid).to.eql(true)
      })
      ...
      it('checks for blank rows', () => {
        data = StringIO.new('"","",')
        const validator = new CsvlintValidator(data, "header" => false)
      ...
      it('should presume a header unless told otherwise', () => {
        stream = "1,2,3\r\n"
        const validator = new CsvlintValidator(StringIO.new(stream))
...
//   stream = "\"a\",“b“,\"c\"" "\r\n"
//   const validator = new CsvlintValidator(stream)
...
// })
      ...
      it('returns line break errors if incorrectly specified', () => {
        ...
        stream = "\"a\",\"b\",\"c\"\n"
        const validator = new CsvlintValidator(StringIO.new(stream), {"lineTerminator" => "\r\n"})
        expect(validator.isValid).to.eql(false)
        expect(validator.errors.count).to eq(1)
...
describe('when detecting headers', () => {
  ...
  it('should look in CSV options to detect header', () => {
    ...
    const validator = new CsvlintValidator("http://example.com/example.csv", opts)
    expect( validator.header? ).to.eql(true)
    opts = {
      "header" => false
  }
...
it('should be valid if we have a dialect and the data is from the web', () => {
  ...
  stub_request(:get, "http://example.com/example.csv").to_return(:status => 200, :headers=>{"Content-Type" => "text/csv"}, :body => File.read(File.join(File.dirname(__FILE__),'..','features','fixtures','valid.csv')))
  const validator = new CsvlintValidator("http://example.com/example.csv", {"header"=>true})
  expect( validator.isValid ).to.eql(true)

  stub_request(:get, "http://example.com/example.csv").to_return(:status => 200, :headers=>{"Content-Type" => "text/csv"}, :body => File.read(File.join(File.dirname(__FILE__),'..','features','fixtures','valid.csv')))
  const validator = new CsvlintValidator("http://example.com/example.csv", {"header"=>false})
  expect( validator.isValid ).to.eql(true)
})
...
it('can get line break symbol', () => {
  const validator = new CsvlintValidator("http://example.com/crlf.csv")
...
:headers=>{"Content-Type" => "text/csv; header=present"},
:body => File.read(File.join(File.dirname(__FILE__),'..','features','fixtures','valid.csv')))
  const validator = new CsvlintValidator("http://example.com/example.csv", {}, nil, limit_lines: 2)
  expect( validator.isValid ).to.eql(true)
  data = validator.data
...
describe('with a lambda', () => {
  ...
  @count = 0
    ...
    const validator = new CsvlintValidator(File.new(File.join(File.dirname(__FILE__),'..','features','fixtures','valid.csv')), {}, nil, { lambda: mylambda })
...
//       :body => File.read(File.join(File.dirname(__FILE__),'..','features','fixtures','valid.csv')))
//
//   const validator = new CsvlintValidator("http://example.com/redirect")
//   expect( validator.isValid ).to.eql(true)
// })
*/
})
